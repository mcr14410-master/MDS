...
export default function FixtureDocumentsSection({ fixture, onUpdate }) {
  ...
  const handleSetPrimary = async (doc) => {
    try {
      await axios.put(`/api/fixtures/documents/${doc.id}`, {
        is_primary: true,
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  {imageDoc ? (
                    <button
                      ...
                      className="w-12 h-12 rounded-md overflow-hidden bg-gray-100 dark:bg-gray-900 flex-shrink-0 hover:ring-2 hover:ring-indigo-500 transition-all relative group"
                      title="Vergrößern"
                    >
                      <AuthImage
                        apiPath={`/api/fixtures/documents/${doc.id}/view`}
                        alt={doc.file_name}
