...
export default function FixtureStorageSection({ fixture, onUpdate }) {
  const { 
    locations,
    fetchLocations,
  ...
  // Alle aktiven Lagerorte anzeigen
  ...
  const handleAddToStorage = async () => {
    ...
    setLoading(true);
    try {
      await axios.post('/api/storage/items/fixture', {
      ...
      toast.success('Vorrichtung eingelagert');
  ...
  const handleRemoveFromStorage = async (storageItemId) => {
    ...
    try {
      await axios.delete(`/api/storage/items/fixture/${storageItemId}`);
  ...
  const storageLocations = fixture.storage_locations || [];
      ...
      {storageLocations.length === 0 ? (
        ...
        <div className="space-y-3">
          {storageLocations.map((loc) => (
            <div 
              key={loc.storage_item_id || loc.id}
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <div className="text-lg font-bold text-gray-900 dark:text-white">
                    {Number(loc.quantity_new || 0) + Number(loc.quantity_used || 0) + Number(loc.quantity_reground || 0) || loc.total_quantity || 0}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">Stück</div>
                </div>
                
                <button
                  onClick={() => handleRemoveFromStorage(loc.storage_item_id || loc.id)}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 disabled:opacity-50"
                  >
