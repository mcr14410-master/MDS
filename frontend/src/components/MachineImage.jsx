// frontend/src/components/MachineImage.jsx
import { useState, useEffect } from 'react';
import axios from '../utils/axios';
import { API_ENDPOINTS } from '../config/api';

/**
 * MachineImage - Zeigt ein Maschinenfoto an
 * Lädt das Bild mit Auth-Header und zeigt es als Blob-URL an
 */
export default function MachineImage({ documentId, alt, className }) {
    ...
    const loadImage = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_ENDPOINTS.MACHINE_DOCUMENTS}/${documentId}/view`,
          { responseType: 'blob' }
        );
        ...
        console.error('Error loading machine image:', err);
        setError(true);
      } finally {
        setLoading(false);
      }
