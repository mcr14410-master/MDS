...
CREATE TABLE tool_master (
  ...
  tool_number VARCHAR(50) NOT NULL UNIQUE,
    COMMENT 'Eindeutige Werkzeugnummer (z.B. T001, WZ-2024-001)',
...
CREATE INDEX idx_tool_master_number ON tool_master(tool_number);
...
INSERT INTO tool_master (
  tool_number, tool_name, diameter, length, flutes,
...
## Phase 5: Integration & Reports (Tag 10)

**Ziel:** Verknüpfung mit Tool Lists, Reports, Dashboard Widgets
...
```
Als Fertigungsleiter möchte ich:
- Beim Erstellen einer Tool List Werkzeuge aus dem Storage System wählen
- Sehen welche Tools in welchen Programmen verwendet werden
- Dashboard mit Low-Stock Alerts
- Reports: Bestandswert, Verbrauch, Bestellhistorie
- Export zu Excel
```
...
**Erweiterungen:**
```javascript
// src/controllers/toolListsController.js (erweitern)
- getToolsForList() // GET /api/tools/available (mit Storage Info)
- linkToolFromStorage() // POST /api/tool-lists/:id/items/from-storage
  // Erstellt tool_list_item und verknüpft mit storage_item
...
**Business Logic:**
- Bei tool_list_item Erstellung → optional storage_item verknüpfen
- Verbrauchs-Tracking: stock_movements nach reference_type='tool_list' filtern
- Inventory Value: SUM(storage_items.quantity * supplier_items.unit_price)
...
- Anzahl Tools unter Reorder Point
- Top 5 kritische Tools
- Link zu Low Stock Page

// src/components/dashboard/InventoryValueWidget.jsx
- Gesamtwert aller Werkzeuge
- Aufschlüsselung nach Kategorie
- Trend (falls historische Daten)

// src/components/dashboard/RecentOrdersWidget.jsx
...
- Tab-System: Inventory, Consumption, Purchase History
- Date Range Filter
- Export Buttons (CSV, Excel, PDF)
- Charts (Recharts): Verbrauch über Zeit, Bestandswert

// src/components/reports/InventoryReport.jsx
- Tabelle: Tool, Location, Quantity, Value
- Total Value
- Gruppierung nach Location/Category

// src/components/reports/ConsumptionReport.jsx
- Tabelle: Tool, Issued Quantity, Date Range
- Chart: Verbrauch über Zeit
- Top Verbraucher

// src/components/reports/PurchaseHistoryReport.jsx
- Tabelle: Order Number, Supplier, Date, Total, Status
- Filter nach Supplier, Status
- Export
```

**Tool Lists Integration:**
```javascript
// src/components/tools/ToolListForm.jsx (erweitern)
- "Add from Storage" Button
- Modal mit Tool-Suche (aus storage_items)
- Beim Auswählen: Daten aus storage_item übernehmen
- Optional: Quantity aus Storage entnehmen
```

### Test Szenarien

```http
### GET Tools for Tool List (with Storage Info)
GET http://localhost:5000/api/tools/available?program_id=1

### LINK Tool from Storage to Tool List
POST http://localhost:5000/api/tool-lists/1/items/from-storage
{
  "storage_item_id": 1,
  "tool_number": "T01",
  "take_from_stock": true,
  "quantity": 1
}

### GET Inventory Value Report
GET http://localhost:5000/api/reports/inventory-value

### GET Consumption Report
GET http://localhost:5000/api/reports/consumption?from=2025-01-01&to=2025-01-31

### GET Low Stock Report
GET http://localhost:5000/api/reports/low-stock
```

**Deliverable:** ✅ Integration mit Tool Lists + Reports + Dashboard
...
| **Phase 2** | 12-14h | Tool Management Core (Storage Assignment, Stock) |
| **Phase 3** | 4-5h | Supplier Management (Stammdaten, Verknüpfung) |
| **Phase 4** | 6-8h | Bestellwesen (Purchase Orders, Receiving) |
| **Phase 5** | 4-5h | Integration (Tool Lists, Reports, Dashboard) |
| **GESAMT** | **36-44h** | **Vollständiges Tool Management System** |
